import pygame  # pygame 라이브러리       #새로운 fake적 개체 추가, 배경음악 넣기
# 필요한 라이브러리들
import random
...
def runGame():
    ...
    global bullet, enemy, fake, bullet2  # -------------------------------------------
    ...
    enemy_speed = 3
    # fake 좌표값           #--------------------------------------------------------
    fake_x = random.randrange(0, pad_width - enemy_width)
    fake_y = 0
    fake_speed = 5
    # --------------------------------------------------------------
    ...
    while not ongame:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:  # 키를 눌렀을 때
                if event.key == pygame.K_LEFT:  # 왼쪽 방향키
                    x_change -= 5
                elif event.key == pygame.K_a:  # 왼쪽 방향키 2  (_______________________----------------------________)
                    x2_change -= 5
        ...
        drawScore(shotcount)
        drawHeart1(player1IsShot)  # 목숨 객체 생성_____________________________________________________________
        ...
        if fake_y > pad_height:  # fake 화면 밖으로 지나갔으면
            fake_x = random.randrange(0, pad_width - enemy_width)
            fake_y = 0
...
def initGame():
    ...
    fighter2 = pygame.image.load('fighter2.png')  # 플레이어2 이미지__________________________
    enemy = pygame.image.load('enemy.png')
    fake = pygame.image.load('fake.png')
